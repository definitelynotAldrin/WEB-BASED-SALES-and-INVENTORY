...
while ($row = $result_order_details->fetch_assoc()) {
    ...
    // Check if item already exists within this order to avoid duplication
    if (isset($orders[$order_id]['items'][$item_name])) {
        // If item exists, add the quantity
        $orders[$order_id]['items'][$item_name] += $quantity;
    } else {
        // If item doesn't exist, set the initial quantity
        $orders[$order_id]['items'][$item_name] = $quantity;
    }
}

// Format items for display with quantities
foreach ($orders as &$order) {
    $formatted_items = [];
    foreach ($order['items'] as $item_name => $quantity) {
        $formatted_items[] = "{$item_name} ({$quantity})";
    }
...
<head>
    ...
    <style>
        ...
        <?php if ($reportOptions['salesReports']): ?> #summary-reports, #orders-reports, #menu-reports, #daily-sales-report { display: block; } <?php endif; ?>
...
<body onload="window.print(); window.onafterprint = function() { window.close(); }">
    <a href="../public/sales_report.php" class="return-button">Return</a>
