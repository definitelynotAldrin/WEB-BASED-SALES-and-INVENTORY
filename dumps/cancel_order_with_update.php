...
if (isset($_POST['order_id'])) {
    ...
    try {
        // Step 0: Fetch and store the current `order_status` of the order
        ...
        $orderStatusResult = $getOrderStatusStmt->get_result();

        if ($orderStatusResult->num_rows > 0) {
            $orderStatusRow = $orderStatusResult->fetch_assoc();
            $previousOrderStatus = $orderStatusRow['recent_status']; // Store the previous status
            ...
            if ($orderResult->num_rows > 0) {
                ...
                if ($result->num_rows > 0) {
                    ...
                    if ($row['count'] == 0) {
                        ...
                        // Roll back to the previous order status if items still exist
                        ...
                        $updateOrderStatusStmt->bind_param("si", $previousOrderStatus, $orderID);
            ...
            throw new Exception('Order status not found for the given order ID.');
        }
    } catch (Exception $e) {
        // Rollback on error
        $conn->rollback();
