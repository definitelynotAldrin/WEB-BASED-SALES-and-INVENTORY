...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($username)) {
        ...
        header("Location: ../public/login_admin.php?error=$errorMsg&$data");
        exit();
    ...
    $sql = "SELECT * FROM accounts WHERE account_username = ? AND user_role = 'user_admin' AND account_status = 'active'";
    ...
    if ($result->num_rows == 1) {
        ...
        if (password_verify($password, $row['account_password'])) {
            ...
            header("Location: ../public/index.php?success=Successfully logged in as admin.");
            exit();
        } else {
            // Invalid password
