...
try {
    ...
    $query = "INSERT INTO messages (user_role, text_message, message_time, message_date) VALUES (?, ?, NOW(), CURDATE())";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("ss", $user_role, $text_message);
    $stmt->execute();

    if ($stmt->affected_rows > 0) {
        // Fetch the most recent messages to update the chat box
        $fetch_query = "SELECT user_role, text_message, CONCAT(message_date, ' ', message_time) AS timestamp FROM messages ORDER BY id DESC LIMIT 10";
